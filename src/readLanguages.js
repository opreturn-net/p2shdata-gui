...
async function setLanguagesJson() {
    ...
    try {
        ...
        if (textLanguages_new[textLanguages_new.settings.selected_language] == undefined)
            textLanguages_new[textLanguages_new.settings.selected_language] = {};
        // check if all keys are present in the selected language
        for (let i = 0; i < keys.length; i++) {
            if (textLanguages_new[textLanguages_new.settings.selected_language][keys[i]] == undefined) {
                textLanguages_new[textLanguages_new.settings.selected_language][keys[i]] = textLanguages.english[keys[i]];
            }
        }
